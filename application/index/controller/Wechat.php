...
class Wechat extends Base
{
    ...
    public function pay()
    {
        ...
        $out_trade_no = session('order_num'); //商户订单号(自定义)            
        $paymoney = session('sumtotal'); //支付金额(分)
        //echo $paymoney;die;
        ...
        if($result['result_code']=='SUCCESS' && $result['return_code']=='SUCCESS') {
            $url = $result["code_url"];
            $this->assign(['url'=>$url,'out_trade_no'=>$out_trade_no,'paymoney'=>$paymoney]);
        }else{
            $this->error('参数错误'); 
        }
        return $this->fetch();
