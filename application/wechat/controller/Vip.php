...
class Vip extends Base
{
    ...
    {
        if (request()->isPost()) {
            $headimgurl = json_decode(input('headimgurl'),true);
            dump($headimgurl);die;
            $user = session('wechat.original');

            $find = db('wx_user')->where('openid',$user['openid'])->find();
         
            if ($find) {
                $update = db('wx_user')->field('headimgurl')->where('openid',$user['openid'])->update($headimgurl);
                if ($update) {
                    $data['code'] = 1;
                    $data['imgurl'] = $headimgurl;
                    $data['msg']  = '成功';
                }else{
                    $data['code'] = 0;
                    $data['msg'] = "操作失败";
